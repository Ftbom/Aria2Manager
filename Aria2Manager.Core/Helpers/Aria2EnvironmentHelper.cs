...
{
    public static class Aria2EnvironmentHelper
    {
        // 预设的aria2.conf内容
        private const string ConfTemplate = @"# 日志
log=aria2.log
log-level=error

## 文件保存相关 ##
# 文件的保存路径(可使用绝对路径或相对路径), 默认: 系统下载目录
dir={DownloadDir}
...
# NTFS(Microsoft)建议使用falloc, EXT3/4建议trunc, MAC 下需要注释此项
...
";
        ...
        public static void InitializeAria2Environment()
        {
            ...
            string confPath = Path.Combine(aria2Dir, "aria2.conf");
            string sessionPath = Path.Combine(aria2Dir, "aria2.session");
            if (!Directory.Exists(aria2Dir))
            {
                Directory.CreateDirectory(aria2Dir);
            }
            //创建aria2.session空文件
